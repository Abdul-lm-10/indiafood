...
const FAQ = () => {
  ...
  const leftFAQs = data.slice(0, half);
  const rightFAQs = data.slice(half);

  // Track active indexes for left and right columns
  ...
  const renderFAQItems = (faqList, activeIndex, side) => (
    faqList.map((faq, index) => (
      ...
      >
        <div
          className="faq-header"
          onClick={() => handleClick(index, side)}
          style={{
            padding: '1.5rem',
            cursor: 'pointer',
            backgroundColor: activeIndex === index ? '#198754' : '#fff',
    ...
    <div className="container py-5">
      ...
      <div className="row">
        <div className="col-md-6">
          {renderFAQItems(leftFAQs, activeIndexLeft, 'left')}
        </div>
        <div className="col-md-6">
          {renderFAQItems(rightFAQs, activeIndexRight, 'right')}
        </div>
      </div>
    </div>
  );
